"""
This module contains functions  related to the preprocessment of the dataset,
so that it can be given as input to our machine learning models.
"""
import random
import numpy as np
import settings
...
def datarate_matrix(scenario_frame):
    """
    Computes a matrix representing the datarate generated by the users from a frame representing the same data
    """
...
def drones_matrix(results_line):
    """
    Computes a matrix representing the location of the drones with sparse encoding.
    """
...
def build_input_structure_transformation(model_structure, model_prediction, list_scenarios, list_topologies,function, angle):
    """
    Auxiliary function that returns a structure to be given as an input to the model, after a transformation has been applied.
    """
    index_model_structure = 0
    model_structure_transformed = []
    model_prediction_transformed = []
    index_scenario = 0
    for _scenario_id in list_scenarios:
        ...
        for _topology_id in list_topologies[index_scenario]:
...
def build_input_struct_channels_last_matrix(input_entry):
    """
    Auxiliary function that constructs one input element of the structute to be given as input to the ML model.
    In this case, the channels of the image come last.
    """
...
def build_input_structure_channels_last(input_struct):
    """
    Function that constructs a structure that should be given as an input to our machine learning model.
    The structure constructed by this function has the specificity that its channels come last.
    """
    new_struct = []
    for input_entry in input_struct:
        new_struct.append(build_input_struct_channels_last_matrix(input_entry))
    return np.array(new_struct)

def separate_qualities(qualities_list):
    """
    Separates different qualities from qualities_list structure.
    """
    throughput = []
    delay = []
    jitter = []
    ...
    for qualities in qualities_list:
        throughput.append(qualities[0])
        delay.append(qualities[1])
        jitter.append(qualities[2])
        pdr.append(qualities[3])


    return np.array(throughput), np.array(delay), np.array(jitter), np.array(pdr)

def build_topologies_train_list(scenarios):
    """
    Build train set when our entire dataset is to be divided by topologies
    """
    top_list = []
...
def build_topologies_validation_n_test_list(train_list):
    """
    Builds validation and train set, when our entire dataset is to be divided by topologies
    """
    return_list = []
    validation_list = []
    test_list = []
